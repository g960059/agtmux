...
- FR-048 `[MVP]`: `list-panes` の default 出力は scanability 優先とし、managed pane は `title + activity_state + relative_time`、unmanaged pane は `current_cmd` を表示する。`activity_state` の表示値は `Running` / `Idle` / `Waiting`（`WaitingInput` / `WaitingApproval` は `Waiting` に正規化）とし、`@N/@M/%N` の内部 ID は default 非表示とする。
- FR-049 `[MVP]`: CLI list 系（`list-panes` / `list-windows` / `list-sessions`）は `--context=auto|off|full` を提供し、default は `auto` とする。`off` は `cwd` / `git branch` と mixed marker を非表示にする。`full` は出力行を増やさず、既存の各表示行へ inline で `cwd` と `git branch` を常時表示する（`list-panes`: 各 pane 行、`list-windows`: 各 window 行、`list-sessions`: 各 session 行）。`list-panes --context=full` では session/window header の集約 context 表示は `auto` と同じまま維持し、pane 行のみが `full` で詳細化される。`auto` で pane 行に context suffix を出すのは「差分がある」または「baseline が mixed」のフィールドのみとする。`list-windows` / `list-sessions` の `auto` は window/session 行の context を常時表示し、親 header との差分抑制は行わない。
- FR-049a `[MVP]`: `list-panes` 出力で session が 1 window の場合、window header 行は常に省略する。session が 2 windows 以上の場合は window header 行を必ず出力する。
...
- FR-068 `[Post-MVP]`: agent process exit / forced termination / pane return-to-shell 後は、その exact pane row を no-agent shell truth (`presence=unmanaged`, `provider=null`) へ demote しなければならない。`current_cmd=zsh` のような shell pane に stale managed/provider/activity truth を残すことは禁止する。heuristic fallback は managed title/provider/activity を保持する理由にならない。
- FR-069 `[Post-MVP]`: explicit `--tmux-socket <path>` targeting は launch context に依存せず authoritative でなければならない。特に codesigned app / XCUITest から child daemon として起動された場合でも、same-user shell からの daemon 起動と同じ tmux inventory / `ui.bootstrap.v2` truth を返さなければならない。app-launched context だけが empty bootstrap (`total=0`) に崩れることは禁止する。
