...
- Contract note: shell bootstrap → managed promotion at the same visible location keeps `pane_instance_id` stable but may legitimately change `session_key` from `shell:%pane` to `<provider>:%pane`
