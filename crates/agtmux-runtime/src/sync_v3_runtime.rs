...
use agtmux_core_v5::sync_v3::{
    ...
    PresenceV3, ProviderRawEnvelopeV3, SyncV3PaneSnapshot, ThreadBlockingV3, ThreadExecutionV3,
    ThreadFlagsV3, ThreadLifecycleV3, ThreadStateV3, TurnOutcomeV3, TurnStateV3, UiBootstrapV3,
};
...
impl SyncV3LiveState {
    ...
    pub fn build_bootstrap(
        ...
        let panes = inventory
            .into_iter()
            .filter_map(|tmux_pane| {
                let pane_instance_id =
                    resolve_inventory_pane_instance_id(tmux_pane, generation_tracker)?;

                let snapshot = if let Some(reducer) = self.reducers.get(&tmux_pane.pane_id) {
                    ...
                    build_unmanaged_snapshot(tmux_pane, pane_instance_id, now)
                };
                ...
                match snapshot.validate() {
                    ...
                    Err(err) => {
                        tracing::warn!(
                            ...
                            "dropping invalid ui.bootstrap.v3 pane: {err}"
                        );
                        None
                    }
                }
...
mod tests {
    ...
    fn build_bootstrap_uses_codex_v3_truth_instead_of_waiting_input_collapse() {
        ...
        let payload = live.build_bootstrap(&[], &panes, &tracker, ts(11));
    ...
    fn build_bootstrap_emits_unmanaged_row_with_strict_identity_when_no_v3_truth_exists() {
        ...
        let payload = live.build_bootstrap(&[], &panes, &tracker, ts(12));
        payload.validate().expect("payload should validate");
    ...
    fn managed_fallback_does_not_reuse_collapsed_v2_activity_state() {
        ...
        let payload = live.build_bootstrap(&[&managed], &panes, &tracker, ts(12));
        payload.validate().expect("payload should validate");
